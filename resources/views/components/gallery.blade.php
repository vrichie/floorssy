<div id="gallery" class="w-full flex flex-col items-center justify-between tablet:justify-evenly my-10">

    <h1 class="text-5xl tablet:text-4xl phone:text-3xl laptop:w-4/5 tablet:w-11/12 phone:w-11/12  my-4 text-slate-800 font-medium font-sans w-2/3 text-left">
        <b class="font-medium"> Our </b> <span class="text-amber-400">Specialty .</span> 
    </h1>

    <div id="gallery_wrapper" class="relative tablet:h-4/5 laptop:w-4/5 tablet:w-11/12 m-auto h-4/5 w-2/3 phone:none  ">

        <div id="top_right" class="phone:w-full phone:h-1/5">
            <div class="wrapper flex items-center justify-center">
                <img src="{{secure_asset('images/laminate.jpg')}}" alt="">
                <div class="text flex items-center justify-center bg-gray-900/70 phone:opacity-100">
                    <h1 class="text-5xl text-slate-100">
                        laminate
                    </h1>

                </div>
            </div>

        </div>

        <div id="bottom_right" class="phone:w-full phone:h-1/5">
            <div class="wrapper flex items-center justify-center">
                <img src="{{secure_asset('images/carpet.jpg')}}" alt="">
                <div class="text flex items-center justify-center bg-gray-900/70 phone:opacity-100">
                    <h1 class="text-5xl text-slate-100">
                        Carpets
                    </h1>

                </div>
            </div>

        </div>


        <div id="middle_card" class="phone:w-full phone:h-1/5">
            <div class="wrapper flex items-center justify-center">
                <img src="{{secure_asset('images/toilet.jpg')}}" alt="">
                <div class="text flex items-center justify-center bg-gray-900/70 phone:opacity-100">
                    <h1 class="text-5xl text-slate-100">
                        Stone
                    </h1>

                </div>
            </div>


        </div>





        <div id="top_left" class="phone:w-full phone:h-1/5">
            <div class="wrapper flex items-center justify-center">
                <img src="{{secure_asset('images/kitchen.jpg')}}" alt="">
                <div class="text flex items-center justify-center bg-gray-900/70 phone:opacity-100">
                    <h1 class="text-5xl text-slate-100">
                        Kitchen tile
                    </h1>

                </div>
            </div>

        </div>

        <div id="bottom_left" class="phone:w-full phone:h-1/5">
            <div class="wrapper flex items-center justify-center">
                <img src="{{secure_asset('images/floorr.jpg')}}" alt="">
                <div class="text flex items-center justify-center bg-gray-900/70 phone:opacity-100">
                    <h1 class="text-5xl text-slate-100">
                        Floor tile
                    </h1>

                </div>
            </div>

        </div>


    </div>


    <div id="mobile_wrapper" class=" w-11/12  flex flex-col items-center justify-evenly ">
        <div  class="phone:w-full phone:h-1/5 my-2">
            <div class="wrapper flex items-center justify-center">
                <img src="{{secure_asset('images/laminate.jpg')}}" alt="">
                <div class="text flex items-center justify-center bg-gray-900/70 phone:opacity-100">
                    <h1 class="text-3xl  text-slate-100">
                        laminate
                    </h1>

                </div>
            </div>

        </div>

        <div  class="phone:w-full phone:h-1/5 my-2">
            <div class="wrapper flex items-center justify-center">
                <img src="{{secure_asset('images/carpet.jpg')}}" alt="">
                <div class="text flex items-center justify-center bg-gray-900/70 phone:opacity-100">
                    <h1 class="text-3xl text-slate-100">
                        Carpets
                    </h1>

                </div>
            </div>

        </div>


        <div  class="phone:w-full phone:h-1/5 my-2">
            <div class="wrapper flex items-center justify-center">
                <img src="{{secure_asset('images/toilet.jpg')}}" alt="">
                <div class="text flex items-center justify-center bg-gray-900/70 phone:opacity-100">
                    <h1 class="text-3xl text-slate-100">
                        Stone
                    </h1>

                </div>
            </div>


        </div>





        <div  class="phone:w-full phone:h-1/5 my-2">
            <div class="wrapper flex items-center justify-center">
                <img src="{{secure_asset('images/kitchen.jpg')}}" alt="">
                <div class="text flex items-center justify-center bg-gray-900/70 phone:opacity-100">
                    <h1 class="text-3xl text-slate-100">
                        Kitchen tile
                    </h1>

                </div>
            </div>

        </div>

        <div  class="phone:w-full phone:h-1/5 my-2">
            <div class="wrapper flex items-center justify-center">
                <img src="{{secure_asset('images/floorr.jpg')}}" alt="">
                <div class="text flex items-center justify-center bg-gray-900/70 phone:opacity-100">
                    <h1 class="text-3xl text-slate-100">
                        Floor tile
                    </h1>

                </div>
            </div>

        </div>

    </div>




    <!-- It is quality rather than quantity that matters. - Lucius Annaeus Seneca -->
</div>